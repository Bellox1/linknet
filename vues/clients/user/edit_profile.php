...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["profile_picture"]["name"])) {
        ...
        $target_dir = "../../back-office/uploads/";
        $filename = $user["username"].$user_id.".jpg";
        $target_file = $target_dir . $filename;
        move_uploaded_file($_FILES["profile_picture"]["tmp_name"], $target_file);

        $stmt = $conn->prepare("UPDATE users SET bio = ?, profile_picture = ? WHERE id = ?");
        $stmt->execute([$bio, $filename, $user_id]);
    } else {
...
?>
    ...
    <h2>Edit Profile</h2>
    <img src="/vues/back-office/uploads/<?php echo $user['profile_picture'] ?: 'default_profile.jpg'; ?>" alt="Profile Picture">
