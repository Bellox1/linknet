...
try {
    ...
    // 1. Infos utilisateur
    $stmt = $conn->prepare("SELECT id, username, email, bio, profile_picture, created_at FROM users WHERE id = ?");
    ...
    // 2. Amis
    $stmt = $conn->prepare("SELECT users.id, users.username, users.profile_picture FROM friends 
